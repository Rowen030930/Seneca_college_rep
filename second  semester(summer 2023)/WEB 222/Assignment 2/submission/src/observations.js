...
function getTotalResults(data) {
  ...
  return data.total_results; // or return data['total_results'];
}
...
function observationSummary(data) {
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    const observedOn = observation.observed_on_details.date;
    console.log(`ID-${id} "${speciesGuess}" (Date: ${observedOn})`);
  }
}
...
function observationSummary2(data) {
  ...
  results.forEach((observation) => {
    const { id, species_guess, observed_on_details } = observation;
    const observedOn = observed_on_details.date;
    console.log(`ID-${id} "${species_guess}" (Date: ${observedOn})`);
  });
}
...
//       : null;
//     if (observationGeoPrivacy === geoPrivacy) {
...
function observationsByGeoPrivacy(data, geoPrivacy) {
  ...
  for (const observation of results) {
    const observationGeoPrivacy = observation.geoprivacy
      ? observation.geoprivacy.toLowerCase()
      : null;
    if (observationGeoPrivacy === (geoPrivacy ? geoPrivacy.toLowerCase() : null)) {
      filteredObservations.push(observation);
    }
  }
...
function transformObservation(original) {
  return {
    ...
    coords: original.location.split(',').map(Number).reverse(),
...
function transformObservations(data) {
  ...
  for (var i = 0; i < data.results.length; i++) {
    var transformedObservation = transformObservation(data.results[i]);
...
function getPlaces(data) {
  return data.results.map((observation) => {
    const { place_ids } = observation;
    const result = {};

    place_ids.forEach((placeId) => {
      result[placeId] = `https://www.inaturalist.org/observations?place_id=${placeId}`;
    });
...
function getUserStats(data) {
  const users = data.results.map((result) => result.user);

  const totalCount = users.length;
  const observationsTotal = users.reduce((total, user) => total + user.observations_count, 0);
  const journalsTotal = users.reduce((total, user) => total + user.journal_posts_count, 0);
  const speciesTotal = users.reduce((total, user) => total + user.species_count, 0);

  const averages = {
    observations: observationsTotal / totalCount,
    journals: journalsTotal / totalCount,
    species: speciesTotal / totalCount
  };
...
function extractTimeZones(data) {
  ...
  data.results.forEach((result) => {
    const { created_time_zone, observed_time_zone } = result;

    if (!timeZones.includes(created_time_zone)) {
      timeZones.push(created_time_zone);
    }

    if (!timeZones.includes(observed_time_zone)) {
      timeZones.push(observed_time_zone);
    }
  });
...
function extractTimeZones2(data) {
  ...
  data.results.forEach((result) => {
    ...
    timeZones.add(created_time_zone);
    timeZones.add(observed_time_zone);
  });
