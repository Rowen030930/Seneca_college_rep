...
function createMetaTag(name, content) {
  ...
  const hasSpace = /\s/.test(trimmedContent);
  const is_space = /\s/.test(trimmedName);

  const nameAttr = is_space ? `"${trimmedName}"` : trimmedName;
  const contentAttr = hasSpace ? `"${trimmedContent}"` : trimmedContent;

  return `<meta name=${nameAttr} content=${contentAttr}>`;
}
...
function parseDateString(value) {
  ...
  if (isNaN(year) || isNaN(month) || isNaN(day)) {
    throw new Error('Invalid date string. Year, month, or day is not a valid number.');
  }
...
function toDateString(value, format) {
  if (!(value instanceof Date)) {
    throw new Error('Invalid date object. Expected a Date instance.');
  }
  ...
  if (!validFormats.includes(format)) {
    throw new Error(
      'Invalid date format. Expected one of the following formats: YYYY-MM-DD, DD-MM-YYYY, MM-DD-YYYY'
    );
  }
...
function normalizeDuration(value) {
  const durationRegex = /^(\d+):(\d+):(\d+)$|^(\d+)h(\d+)m(\d+)s$/;
...
function formatDurations(...values) {
  const validDurations = values.filter((value) => {
    return normalizeDuration(value) !== null;
  });

  const formattedDurations = validDurations.map((value) => {
...
function generateLicenseBadge(licenseCode) {
  if (licenseCode === 'CC-BY') {
    return '<a href="https://creativecommons.org/licenses/by/4.0/"><img src="https://licensebuttons.net/l/by/4.0/88x31.png" alt="Creative Commons Attribution License"></a>';
  } else if (licenseCode === 'CC-BY-NC') {
    return '<a href="https://creativecommons.org/licenses/by-nc/4.0/"><img src="https://licensebuttons.net/l/by-nc/4.0/88x31.png" alt="Creative Commons Attribution-NonCommercial License"></a>';
  } else if (licenseCode === 'CC-BY-SA') {
    return '<a href="https://creativecommons.org/licenses/by-sa/4.0/"><img src="https://licensebuttons.net/l/by-sa/4.0/88x31.png" alt="Creative Commons Attribution-ShareAlike License"></a>';
  } else if (licenseCode === 'CC-BY-ND') {
    return '<a href="https://creativecommons.org/licenses/by-nd/4.0/"><img src="https://licensebuttons.net/l/by-nd/4.0/88x31.png" alt="Creative Commons Attribution-NoDerivs License"></a>';
  } else if (licenseCode === 'CC-BY-NC-SA') {
    return '<a href="https://creativecommons.org/licenses/by-nc-sa/4.0/"><img src="https://licensebuttons.net/l/by-nc-sa/4.0/88x31.png" alt="Creative Commons Attribution-NonCommercial-ShareAlike License"></a>';
  } else if (licenseCode === 'CC-BY-NC-ND') {
    return '<a href="https://creativecommons.org/licenses/by-nc-nd/4.0/"><img src="https://licensebuttons.net/l/by-nc-nd/4.0/88x31.png" alt="Creative Commons Attribution-NonCommercial-NoDerivs License"></a>';
  }
...
function buildQueryString(queryTerm, sortOrder = 'ascending', count) {
  if (count < 10 || count > 200) {
    throw new Error('Invalid count value. Count must be between 10 and 200.');
  }

  if (sortOrder !== 'ascending' && sortOrder !== 'descending') {
    throw new Error('Invalid sortOrder value. Sort order must be "ascending" or "descending".');
  }
