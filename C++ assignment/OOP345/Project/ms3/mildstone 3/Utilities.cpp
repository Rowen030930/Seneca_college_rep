...
namespace seneca {
	...
	string Utilities::extractToken(const string& str, size_t& next_pos, bool& more) {
		...
		string temp = (delimiterPos == string::npos) ? str.substr(next_pos) : str.substr(next_pos, delimiterPos - next_pos);
		trim(temp);

		more = (delimiterPos != string::npos);
		next_pos = more ? delimiterPos + 1 : 0;

		setFieldWidth(max(getFieldWidth(), temp.length()));
