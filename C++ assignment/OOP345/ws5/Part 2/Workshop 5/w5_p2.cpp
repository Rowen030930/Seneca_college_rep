...
int main(int argc, char** argv)
{
	...
	if (argc == 5) {
		...
		if (!bookFile.is_open()) {
			std::cerr << "ERROR: Cannot open book file.\n";
		...
		size_t cnt{ 0 };
		do { 
			std::getline(bookFile, bookLine); 
			if (bookFile) { 
				if (bookLine[0] != '#') { 
					library += seneca::Book(bookLine); 
					cnt++;
				} 
			
			} 

		} while (bookFile && cnt < 4);
	...
	auto updateBookPrice = [usdToCadRate, gbpToCadRate](seneca::Book& book) {
		if (book.country() == "US")
	...
	if (argc > 2) {
		...
		size_t cnt{ 0 };
		do {
			std::getline(movieFile, movieLine);
			if (movieFile) {
				if (movieLine[0] != '#') {
